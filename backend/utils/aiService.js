...
export const chatCompletion = async ({ system, user, temperature = 0.7}) => {
    ...
    if (!c) {
        return {
            ok: false,
            content:
              "AI features are disabled - set GEMINI_API_KEY in the backend .env to enable real AI responses. You can still use the app normally for habit tracking, streaks, and statistics, but AI-generated reports and suggestions will not work until the API key is configured.",
        };
    }
    try {
